...
class DeltaPerformer : public FileWriter {
...
#ifdef UPDATE_ENGINE_FOR_CAR
          if (payload_) {
            cryptor_.Init(payload_->password.data(),
                          payload_->password.size(),
                          PayloadEncryptor::kAES_256,
                          PayloadEncryptor::kDecrypto);
          } else {
            LOG(ERROR) << "payload object is NULL";
          }
#endif  // #ifdef UPDATE_ENGINE_FOR_CAR  
    CHECK(install_plan_);
  }
 ...
 private:
  ...
  // The encrypto
  PayloadEncryptor cryptor_;

  uint64_t split_data_offset_ = 0;
  // split size now
  uint64_t split_size_now_ = 0;
  // split hash calculator
  HashCalculator split_hash_calculator_;
#endif  // #ifdef UPDATE_ENGINE_FOR_CAR
