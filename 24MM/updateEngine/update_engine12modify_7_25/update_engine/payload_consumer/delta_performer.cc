...
#include <puffin/stream.h>//iauto jzz
...
bool DeltaPerformer::PerformReplaceOperation(
  ...
  //iauto ??? jzz{
  // Decrypto before extract data.
  brillo::Blob decrypto_data;
  brillo::Blob input_data(operation.data_length());
  memcpy(input_data.data(), buffer_.data(), operation.data_length());
  TEST_AND_RETURN_FALSE(cryptor_.Decrypto(&input_data, &decrypto_data));

  TEST_AND_RETURN_FALSE(partition_writer_->PerformReplaceOperation(
      operation, decrypto_data.data(), operation.data_length_before_crypto()));
...
bool DeltaPerformer::PerformPuffDiffOperation(const InstallOperation& operation,
  ...
  //     operation, error, buffer_.data(), buffer_.size()));//iauto ??? jzz
//iauto ??? jzz
  ...
  //if (ExtractSignatureMessageFromOperation(operation)) {//iauto ??? jzz
    if (ExtractSignatureMessage()) {
