...
void DownloadAction::PerformAction() {
  ...
  if (install_plan_.is_unboot) {
    LOG(INFO) << "Marking new slot as unbootable";  // LCOV_EXCL_BR_LINE
    if (!boot_control_->MarkSlotUnbootable(install_plan_.target_slot)) {
      // LCOV_EXCL_BR_START
      LOG(WARNING) << "Unable to mark new slot "
                   << BootControlInterface::SlotName(install_plan_.target_slot)
    ...
    LOG(INFO) << "not update soc, not need unbootable";  // LCOV_EXCL_BR_LINE
  }
...
void DownloadAction::StartDownloading() {
  LOG(INFO) << "StartDownloading begin";//iauto jzz
