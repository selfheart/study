...
int Main(int argc, char** argv) {
  ...
  // {   by  iauto.com
  DEFINE_string(firmwares, "",
              "Path to the new firmwares. To pass multiple firmwares, use "
              "a single argument with a colon between paths, e.g. "
              "/path/to/part:/path/to/part2:/path/to/last_part . Path has "
              "to match the order of firmwares_names.");
  DEFINE_string(firmware_names, "",
              "Names of the firmwares. To pass multiple names, use a single "
              "argument with a colon between names, e.g. "
              "name:name2:name3:last_name . Name can not be empty, and it "
              "has to match the order of firmware.");
  DEFINE_string(aes_keys, "",
              "The key is for encrypto for payload data. The key is in "
              "base64 format.");
  DEFINE_string(aes_iv, "",
              "The iv is for encrypto for payload data. The iv is in "
              "base64 format.");
  // } Modify by  iauto.com

  // { modify @iauto.com
  // add model_id
  DEFINE_uint64(model_id, 0x99990000,
              "model_id: region + product type, such as : AU +AVN");
  DEFINE_string(model_list, "NP33A,NP33B",
              "The model_list is for model check.");
  DEFINE_uint64(split_size, 0,
              "split_size: if 0 ,no split");
  // } modify @iauto.com
  ...
  // { Modify by iauto.com
  if (FLAGS_aes_keys.empty()) {
    // LOG(ERROR) << "passwd miss! init passwd: 888888";
    FLAGS_aes_keys = "888888";
  }
  if (FLAGS_model_id == 0x99990000) {
    // LOG(ERROR) << "model_id miss! init model_id: 0x99990000";
  }
  if (FLAGS_model_list.empty()) {
    FLAGS_model_list = "891B,892B";
  }
...
// { Modify by < iauto.com>
  payload_config.base64_password = FLAGS_aes_keys;
  // } Modify by < iauto.com>

  // { modify @iauto.com
  // add model_id
  payload_config.model_id = FLAGS_model_id;
  payload_config.model_list = FLAGS_model_list;
  payload_config.split_size = FLAGS_split_size;
  // } modify @iauto.com
...
// { Modify by  iauto.com
  ...
  // // { Modify by < iauto.com>
  // payload_config.target.LoadPrgHeader();
  // // } Modify by < iauto.com>
