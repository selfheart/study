...
class ChunkProcessor : public base::DelegateSimpleThread::Delegate {
 public:
  ...
  ChunkProcessor(const PayloadVersion& version,
      ...
      : version_(version),
        ...
        aop_(aop),
        password_(password) {}              // modify iauto.com 
 ...
 private:
  ...
  std::string password_;      // modify wujie@iauto.com
...
bool ChunkProcessor::ProcessChunk() {
  ...
  TEST_AND_RETURN_FALSE(diff_utils::GenerateBestFullOperation(
   ...
   // Crypto data with AES-128.
  //
  // { iauto jzz

  // LOG(INFO) << "ProcessChunk offset: " << offset_ << " off size:"
  //              << size_;

  chromeos_update_engine::PayloadEncryptor encryptor;

  const unsigned char* p = reinterpret_cast<const unsigned char*>(password_.c_str());
  // LOG(INFO) << "key:" << password_;
  // LOG(INFO) << "key size:" << password_.size();
  encryptor.Init(p, password_.size(), PayloadEncryptor::kAES_256, PayloadEncryptor::kEncrypto);   // modify 256 zhangyan1
  brillo::Blob encrypto_data;
  encryptor.Encrypto(&op_blob, &encrypto_data);
  aop_->op.set_data_length_before_crypto(op_blob.size());

  aop_->op.set_type(op_type);
  TEST_AND_RETURN_FALSE(aop_->SetOperationBlob(encrypto_data, blob_file_));
  // } iauto jzz
  return true;
}
