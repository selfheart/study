...
bool PayloadFile::Init(const PayloadGenerationConfig& config) {
  ...
  manifest_.set_max_timestamp(config.max_timestamp);
      ...
      // { modify iauto.com
  // add model_id
  manifest_.set_model_id(config.model_id);
  manifest_.set_split_count(0);
  split_size_ = config.split_size;
  LOG(INFO) << "split_size:" << split_size_;
  manifest_.set_model_list(config.model_list);
  // } modify iauto.com
