...
// // Load Prg header
// //
// bool ImageConfig::LoadPrgHeader() {
//     LOG(INFO) << "====>>LoadPrgHeader ";
//   for (PartitionConfig& part : partitions) {
//     std::string prg_file_path = part.path.substr(0, part.path.find('.'));
//     prg_file_path += ".prg_header";
//     LOG(INFO) << "====>>PRGFILE: "<< prg_file_path;
//     if (!utils::FileExists(prg_file_path.c_str())) {
//       LOG(ERROR) << "prg file not exist.";
//       return false;
//     }
//     if (utils::FileSize(prg_file_path.c_str()) != 512) {
//       LOG(ERROR) << "prg file size not equal 512.";
//       return false;
//     }
//     LOG(INFO) << "path:"<<part.path<<"name:"<<part.name;
//     utils::ReadFile(prg_file_path.c_str(), &part.prg_header);
//   }

//   return true;
// }
// // } modify by wujie@iauto.com
