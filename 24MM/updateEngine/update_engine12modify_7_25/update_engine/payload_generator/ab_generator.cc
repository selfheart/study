...
bool ABGenerator::GenerateOperations(const PayloadGenerationConfig& config,
  ...
  TEST_AND_RETURN_FALSE(diff_utils::DeltaReadPartition(aops,
                                                       ...
                                                       config.version,
                                                       blob_file,
                                                       config.base64_password));    // modify iauto.com
  LOG(INFO) << "done reading " << new_part.name;

   TEST_AND_RETURN_FALSE(
      FragmentOperations(config.version, aops, new_part.path, blob_file, config.base64_password)); // modify iauto.com
  ...
  TEST_AND_RETURN_FALSE(MergeOperations(
      aops, config.version, merge_chunk_blocks, new_part.path, blob_file, config.base64_password));  // modify iauto.com
...
bool ABGenerator::FragmentOperations(const PayloadVersion& version,
                                     ...
                                     BlobFileWriter* blob_file,
                                     std::string password) {    // modify iauto.com
    ...
    if (aop.op.dst_extents_size() > 1) {
      ...
      if (IsAReplaceOperation(aop.op.type())) {
        TEST_AND_RETURN_FALSE(SplitAReplaceOp(
            version, aop, target_part_path, &fragmented_aops, blob_file,password));  // modify iauto.com
        continue;
      }
    }
...
bool ABGenerator::SplitAReplaceOp(const PayloadVersion& version,
  ...
  for (int i = 0; i < original_op.dst_extents_size(); i++) {
    ...
    *(new_op.add_dst_extents()) = dst_ext;
    ...
    new_aop.name = base::StringPrintf("%s:%d", original_aop.name.c_str(), i);
    TEST_AND_RETURN_FALSE(
        AddDataAndSetType(&new_aop, version, target_part_path, blob_file,password)); // modify iauto.com
...
bool ABGenerator::MergeOperations(vector<AnnotatedOperation>* aops,
  ...
  for (AnnotatedOperation& curr_aop : new_aops) {
    if (curr_aop.op.data_length() == 0 &&
        ...
        TEST_AND_RETURN_FALSE(
          AddDataAndSetType(&curr_aop, version, target_part_path, blob_file, password)); // modify iauto.com
    }
  }
...
bool ABGenerator::AddDataAndSetType(AnnotatedOperation* aop,
  ...
  if (aop->op.type() != op_type || aop->op.data_length() != blob.size()) {
    // {  iauto.com
    LOG(INFO) << "ABGenerator";
    chromeos_update_engine::PayloadEncryptor encryptor;
    const unsigned char* p = reinterpret_cast<const unsigned char*>(password.c_str());
    encryptor.Init(p, password.size(), PayloadEncryptor::kAES_256, PayloadEncryptor::kEncrypto);     // modify 256 iauto
    brillo::Blob encrypto_data;
    encryptor.Encrypto(&blob, &encrypto_data);
    aop->op.set_data_length_before_crypto(blob.size());

    aop->op.set_type(op_type);
    aop->SetOperationBlob(encrypto_data, blob_file);
