...
class FileDeltaProcessor : public base::DelegateSimpleThread::Delegate {
 public:
  FileDeltaProcessor(const string& old_part,
                     ...
                     const std::string& aes_key_base64)   // modify iauto.com
      : old_part_(old_part),
        new_part_(new_part),
        version_(version),
        ...
        aes_key_base64_(aes_key_base64) {}            // modify iauto.com
 ...
 private:
  ...
  const std::string aes_key_base64_;      // modify iauto.com
...
void FileDeltaProcessor::Run() {
  ...
  if (!DeltaReadFile(&file_aops_,
                     ...
                     version_,
                     blob_file_,
                     aes_key_base64_)) {      // modify iauto.com
  ...
  if (!ABGenerator::FragmentOperations(
          version_, &file_aops_, new_part_, blob_file_, aes_key_base64_)) { // TODO ??? jzz modify jzz iauto.com
...
bool DeltaReadPartition(vector<AnnotatedOperation>* aops,
  ...
  TEST_AND_RETURN_FALSE(DeltaMovedAndZeroBlocks(aops,
                                                ...
                                                &new_visited_blocks,
                                                &old_zero_blocks,
                                                aes_key_base64));   // modify iauto.com
  ...
  if (!new_unvisited.empty()) {
    ...
    file_delta_processors.emplace_back( 
        ...
        soft_chunk_blocks,
        blob_file,
         aes_key_base64);   //TODO ??? jzz modify iauto.com
  }
...
bool DeltaMovedAndZeroBlocks(vector<AnnotatedOperation>* aops,
                             ...
                             ExtentRanges* new_visited_blocks,
                             ExtentRanges* old_zero_blocks,
                             const std::string& aes_key_base64) {   // jzz modify iauto.com
...
bool DeltaReadFile(vector<AnnotatedOperation>* aops,
  ...
  for (uint64_t block_offset = 0; block_offset < total_blocks;
    ...
    std::string password = aes_key_base64;
    LOG(INFO) << "aes_key:" << aes_key_base64;
    const unsigned char* p = reinterpret_cast<const unsigned char*>(password.c_str());
    encryptor.Init(p, password.size(), PayloadEncryptor::kAES_256, PayloadEncryptor::kEncrypto);    // modify 256 zhangyan
    brillo::Blob encrypto_data;
    encryptor.Encrypto(&data, &encrypto_data);
    aop.op.set_data_length_before_crypto(data.size());

    // Write the data
    TEST_AND_RETURN_FALSE(aop.SetOperationBlob(encrypto_data, blob_file));
    // } modify wujie@iauto.com
