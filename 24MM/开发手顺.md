...
http://iredmine.iauto.com/projects/rhine_releasecheck/issues
...
Lexus：
teslin/milestone/PreCV
T1：
rhine_ac8025/milestone/PreCV
T2：
rhine_t2/milestone/PreCV

@所有人 PreCV分支实施提交管制， 需要在提交申请网站上申请，SE review代码+2， 申请网站打圈才会纳入。
http://iredmine.iauto.com/commit_management/index?project_id=rhine_release
...
date "2024-01-08 20:16:20"



date "2024-01-29 12:16:20"
...
setprop persist.iauto.log.switch 31
logcat -G 256m && logcat  -g
logcat -s OTA-MASTER  OTA-COMPOSER  UPDATE_AGENT   TLSProxy update_engine   InstallerHal & 
...
setprop persist.log.tag.IVI-HSM-PROXY  V

setprop persist.log.tag  S
setprop persist.iauto.log.switch 31
logcat -G 256m && logcat  -g
logcat -s OTA-MASTER  OTA-COMPOSER  UPDATE_AGENT   SYS_HWH_UPDATE   IVI-HSM-PROXY  IVI-HSM-PROXY-EXTEND  IVI-HSM-SVC SYS_HWH_UCOM_UU & 
...
setprop persist.iauto.log.switch 31
logcat -G 256m && logcat  -g
logcat -s OTA-MASTER  OTA-COMPOSER  UPDATE_AGENT   SYS_HWH_UPDATE    SYS_HWH_UCOM_UU & 
...
这一套设置好，网络连接上。ACC OFF/ON就应该ready了
  ...
  ![image-20240108172317725](C:\Users\jiangzhuangzhuang\study\study\24MM\开发手顺.assets\image-20240108172317725.png)
...
/data/busybox top -H -b -n 10 -d 1 | grep -iE "update_trigger|update_engine|com.iauto.updateagent|com.iauto.otacomposer|com.iauto.otamaster"

/data/busybox top -H -b -n 10 -d 1 | grep -iE "update_engine"

U盘文件下载 

/data/busybox top -H -b -n 10 -d 1 | grep -iE "com.iauto.otamaster|update_trigger"

/data/busybox top -H -b -n 30 -d 1 | grep -iE "update_trigger"



升级

/data/busybox top -H -b -n 10 -d 1 | grep -iE "update_engine|com.iauto.updateagent|com.iauto.otacomposer"
...
aes_key密钥文件是以二进制存储的，你用openssl前要转换成16进制：xxd -p -c 16 aes_key > aes_key_hex

去base64：base64 -d extra_info.txt > extra_info.bin

openssl aes-128-cbc -d -in extra_info.bin -out decrypted.txt -K "$(cat aes_key_hex)" -iv 12345678901234567890abcdefabcdef -nopad
...
http://wikijs.ci.iauto.com/zh/home/ci/codequality/unittest/agitarone

可以用192.168.80.82和192.168.80.84这2台备用服务器调试



### 6、master有precv没有的

http://gerrit.iauto.com/c/Src/iAutoDroid/packages/service/updateagent/+/234094
